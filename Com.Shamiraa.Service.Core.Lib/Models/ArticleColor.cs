   ...
   public class ArticleColor : StandardEntity, IValidatableObject
    {
       [MaxLength(255)]
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            ArticleColorService service = (ArticleColorService)validationContext.GetService(typeof(ArticleColorService));

            if (service.DbContext.Set<ArticleColor>().Count(r => r._IsDeleted.Equals(false) && r.Id != this.Id && r.Code.Equals(this.Code)) > 0)
            {
                yield return new ValidationResult("Kode sudah ada", new List<string> { "code" });
            }
            if (service.DbContext.Set<ArticleColor>().Count(r => r._IsDeleted.Equals(false) && r.Id != this.Id && r.Name.Equals(this.Name)) > 0)
            {
