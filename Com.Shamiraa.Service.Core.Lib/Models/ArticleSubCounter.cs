...
namespace Com.Shamiraa.Service.Core.Lib.Models
{
    public class ArticleSubCounter : StandardEntity, IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            ArticleSubCounterService service = (ArticleSubCounterService)validationContext.GetService(typeof(ArticleSubCounterService));

            if (service.DbContext.Set<ArticleSubCounter>().Count(r => r._IsDeleted.Equals(false) && r.Id != this.Id && r.Code.Equals(this.Code)) > 0)
            {
